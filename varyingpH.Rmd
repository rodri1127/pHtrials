...
```{r}
data_raw <- read_excel("2023-07-17 varying pH trial 5 (2).xlsx", range = "B28:CU221")
```
...
data_final <- data_raw %>% 
  ...
  mutate(well_status = case_when(well %in% blank_wells ~ "blank",
                                 ...
                                 TRUE ~ "interior")) %>%
  mutate(condition = case_when(well %in% condition_1 ~ "inital of 7.91", TRUE ~ "intial of 8.59")) %>%
