...
data_finaltrial1 <- data_raw1 %>% 
  ...
  mutate(well_status = case_when(well %in% blank_wells ~ "blank",
                                 ...
                                 TRUE ~ "interior")) %>%
  mutate(condition = case_when(well %in% condition_1 ~ "inital of 6.50", TRUE ~ "intial of 5.99")) %>%
...
data_finaltrial3 <- data_raw3 %>% 
  ...
  mutate(well_status = case_when(well %in% blank_wells ~ "blank",
                                 well %in% edge_cells ~ "edge",
                                 TRUE ~ "interior")) %>%
  mutate(condition = case_when(well %in% condition_1 ~ "inital of 4.48", TRUE ~ "intial of 4.07")) %>%
  select(elapsed_mins, well, well_status, OD600, condition)
...
data_finaltrial4 <- data_raw4 %>% 
  ...
  mutate(well_status = case_when(well %in% blank_wells ~ "blank",
                                 well %in% edge_cells ~ "edge",
                                 TRUE ~ "interior")) %>%
  mutate(condition = case_when(well %in% condition_1 ~ "inital of 7.04", TRUE ~ "intial of 7.54")) %>%
  select(elapsed_mins, well, well_status, OD600, condition)
...
allthedata %>%
  ...
  scale_color_manual(values=c("#196F3D", "#3EFF33", "#E4FF33", "#FFF833", "#FDB727", "#FD4027", "#27FD9A", "#27FDED"))
```
