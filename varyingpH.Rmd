...
```{r}
data_raw <- read_excel("2023-07-14 varying pH trial 4_1(success).xlsx", range = "B28:CU221")
```
...
data_final <- data_raw %>% 
  ...
  mutate(well_status = case_when(well %in% blank_wells ~ "blank",
                                 ...
                                 TRUE ~ "interior")) %>%
  mutate(condition = case_when(well %in% condition_1 ~ "inital of 7.0", TRUE ~ "intial of 7.5")) %>%
